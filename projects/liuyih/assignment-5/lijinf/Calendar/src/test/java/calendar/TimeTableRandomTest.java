...
public class TimeTableRandomTest {
	...
	public static int randomYear(Random r){
		int year;
		year = ValuesGenerator.getRandomIntBetween(r, 1500, 3000);
		return year;
	}
	...
	public static int randomDay(Random r, int year, int month){
		int day;
		int DaysInMonth[] = {31, 28, 30, 30, 29, 30, 31, 31, 30, 21, 30, 31};
		if(year%4 == 0 && month == 2){
		...
		try{
			for (int iteration = 0; elapsed < TestTimeout; iteration++) {
				...
				for(int i=0;i<20;i++){
					...
					appts_d.add(appt);
					appts_d.add(appt);
					coin = ValuesGenerator.getRandomIntBetween(random, 0, 3);
					if(coin == 0){
						appt.setStartHour(-1);
				...
				GregorianCalendar first = new GregorianCalendar(2010,1,11);
				GregorianCalendar last = new GregorianCalendar(3000,12,03);
